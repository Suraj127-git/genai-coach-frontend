    ...
    <Animated.View style={{ flex: 1 }} entering={FadeInUp}>
      <Layout style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: Math.max(32, insets.top + 8), paddingBottom: Math.max(32, insets.bottom + 8), alignItems: 'center' }}>
          <Animated.Image source={require('../../assets/split_ai_scene_4.png')} style={[{ width: '100%', height: imgH, marginTop: 8 }, imgStyle]} resizeMode='contain' />
          <Animated.View style={[{ marginTop: 16 }, titleStyle]}>
            <Text category='h4' style={{ textAlign: 'center' }}>Realtime Feedback</Text>
          </Animated.View>
          <Animated.View style={[{ marginTop: 8 }, hintStyle]}>
            <Text appearance='hint' style={{ textAlign: 'center' }}>Record, transcribe, and receive actionable insights to refine your answers.</Text>
          </Animated.View>
        </ScrollView>
        <View style={{ position: 'absolute', right: 24, bottom: Math.max(24, insets.bottom + 12) }}>
          <Animated.View style={btnStyle}>
            <Button style={{ width: 56, height: 56, borderRadius: 28 }} onPress={() => router.push('/onboarding/three')}>→</Button>
          </Animated.View>
        </View>
      </Layout>
    </Animated.View>
